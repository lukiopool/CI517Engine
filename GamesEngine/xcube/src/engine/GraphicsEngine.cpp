...
#define RotatingRectSheet "sprites/RotatingRectangleSheet.png"
#define RotatingRect0 "sprites/RotatingRectangle0.png"
#define Tom "sprites/Tom.png"
#define TomWalk0 "sprites/TomWalk0.png"
#define TomWalk1 "sprites/TomWalk1.png"
#define TomWalk2 "sprites/TomWalk2.png"
#define TomWalk3 "sprites/TomWalk3.png"
...
void GraphicsEngine::loadImages()// loads all the images in their locations. called constantly as the rects can change positions
{

	if (renderer == NULL)
	{
		std::cout << "renderer in load images is null" << std::endl;
	}
...
void GraphicsEngine::addImageToList(int arrayRef)
{
	...
	if (arrayRef == 0)//testing sheet
	{
		imageName[arrayRef] = "RotatingRectangleSheet";
		img[arrayRef] = IMG_LoadTexture(renderer, RotatingRectSheet);/// saves image 0 as being the rotating rect sheet


	}
	else if (arrayRef == 1)// tom the test cube
	{

		imageName[arrayRef] = "Tom";
		img[arrayRef] = IMG_LoadTexture(renderer, Tom);


	}
	else if (arrayRef == 2)//tom the test cube walk anim 0
	{

		imageName[arrayRef] = "TomWalk0";
		img[arrayRef] = IMG_LoadTexture(renderer, TomWalk0);
	...
	else if (arrayRef == 3) 
	{
		...
		imageName[arrayRef] = "TomWalk1";
		img[arrayRef] = IMG_LoadTexture(renderer, TomWalk1);

	}
	else if (arrayRef == 4) // tom the test cube walk anim 2
	{
		imageName[arrayRef] = "TomWalk2";
		img[arrayRef] = IMG_LoadTexture(renderer, TomWalk2);
	}
	else if (arrayRef == 5) // tom the test cube walk anim 3
	{
		imageName[arrayRef] = "TomWalk3";
		img[arrayRef] = IMG_LoadTexture(renderer, TomWalk3);
	}
	...
	SDL_QueryTexture(img[arrayRef], NULL, NULL, &w, &h); // checks the width and height of the image and sets? it to w and h

	imageRect[arrayRef].x; // not set, set in the function below "setImageLocation"
	imageRect[arrayRef].y;
	imageRect[arrayRef].w = w; // 
	imageRect[arrayRef].h = h; //
...
void GraphicsEngine::setImageLocation(int arrayPos, int xpos, int ypos)
{

	imageRect[arrayPos].x = xpos; // not set, set in the function below "setImageLocation"
	imageRect[arrayPos].y = ypos;

}

void GraphicsEngine::renderImage()// be careful, if it tries to render things that dont exist, it'll panic
{
	
	if (renderer == NULL)
	{
		std::cout << "renderer in render images is null" << std::endl;
	}
	
	for (int i = 0; i < 70; i++)
	{
		if (imageName[i] != "")// if the section isnt empty
		{
			SDL_RenderCopy(renderer, img[i], NULL, &imageRect[i]);
	...
	//std::cout << SDL_RenderCopy(renderer, img[0], NULL, &imageRect[0]) << std::endl;
...
void GraphicsEngine::renderPresent()
{
